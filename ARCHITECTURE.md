...
C4Container
    ...
    System_Ext(skillHost, "FlowCharge Core release host", "github.com/FlowChargeApp/flowcharge-core — serves the skill suite as a release .zip, listed through api.github.com")
    ...
    Rel(cli, skillHost, "Lists releases and downloads the skill suite .zip", "REST / JSON, then binary over HTTPS")
...
`Aptabase`, `skillHost` and `ToolDir`.
...
sequenceDiagram
    ...
    participant skillHost as FlowCharge Core release host
    ...
    skillReleaseFetch->>skillHost: GET the releases API
    skillHost-->>skillReleaseFetch: release JSON
    skillContentFetch->>skillHost: GET the newest release asset .zip
    skillHost-->>skillContentFetch: zip bytes
...
| skillPresence | installFormat | direct import | `IntegrationFormat`, `FileWrite`, `InstallContent` | Derives each expected path through `selectPrimaryFormat` and `formatForTarget` over a stub `InstallContent`, never a hand-rolled path guess. The stub carries no `files`, so a `skill-directory` or `rule-directory` format still yields exactly one `FileWrite` per skill and `writes[i]` stays aligned with `skillIds[i]`. OpenCode is the one tool whose write maps to two candidate paths rather than one: the probe also tries the legacy singular `skill/` spelling of the plural `skills/` folder this app writes, and either hit counts as present. |
| skillVersion | toolSignals | injected port call | `FsAccess` | Uses `readTextFile` only, never anything from `FsWriteAccess`. |
| skillVersion | installFormat | direct import | `IntegrationFormat`, `FileWrite`, `InstallContent` | Resolves each present skill's read path through the same `selectPrimaryFormat` and `formatForTarget` pair `skillPresence` uses, and answers the first version that parses. It does not carry `skillPresence`'s OpenCode legacy-folder candidate, so an OpenCode skill found only under the singular `skill/` spelling is reported present and still reads no version. |
...
| installEngine | installFormat | direct import | `IntegrationFormat`, `FileWrite`, `InstallContent` | `selectPrimaryFormat` returns null for a tool with no usable format at the target's scope, so Cursor and Windsurf are skipped at global scope. All four writing formats emit each skill's bundled `SkillContent.files` alongside its body: `skill-directory` puts them under the skill's own folder, while `rule-directory` and the two single-document formats put them in a directory named for the skill beside the rule file or combined document, so two skills sharing one `files[].relativePath` cannot overwrite each other. |
...
| skillReleaseFetch | FlowCharge Core release host | outbound GET with a 10000 ms abort timeout, body capped at 4 MB | `SkillReleaseSummary` | The repository base URL arrives as a parameter, never as a constant in this module. `releasesApiUrl` then picks the route shape from that URL's hostname: `github.com` maps to `https://api.github.com/repos/<owner>/<repo>/releases`, and every other host keeps the Gitea shape `<origin>/api/v1/repos/<owner>/<repo>/releases`. |
...
- `skillReleaseFetch` must hold no repository constant. The repository base URL always
  arrives as a parameter, which is what keeps the two modules free of an import cycle. The
  one host literal it may hold is the API origin a known forge demands — GitHub serves its
  release list from `api.github.com`, not from the repository origin — and that branch must
  read the hostname off the supplied URL, never off a stored repository.
